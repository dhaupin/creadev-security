 ...
 */
export function validateUUIDs(...ids: unknown[]): boolean {
 ...
 */
export function validateBranchName(branch: unknown): branch is string {
  if (typeof branch !== 'string') return false;
  if (branch.length === 0 || branch.length > 255) return false;
  if (!SAFE_BRANCH_REGEX.test(branch)) return false;
 ...
 */
export function safeBranchName(branch: unknown, defaultBranch = 'main'): string {
...
export function validateFilePath(filePath: unknown): filePath is string {
  ...
  if (!SAFE_PATH_REGEX.test(filePath)) return false;
 ...
 */
export function validateFilePaths(paths: unknown): paths is string[] {
...
export function validateSHA(sha: unknown): sha is string {
...
export function validateShortSHA(sha: unknown): sha is string {
 ...
 */
export function validateRepoFullName(fullName: unknown): fullName is string {
 ...
 */
export function parseRepoFullName(fullName: unknown): { owner: string; repo: string } | null {
  if (!validateRepoFullName(fullName)) return null;
 ...
 */
export function validateCommitMessage(message: unknown): message is string {
  if (typeof message !== 'string') return false;
  if (message.length === 0 || message.length > 5000) return false;
  
  // Block null bytes and most control characters (allow newlines, tabs)
  if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(message)) return false;
