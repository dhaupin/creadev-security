...
const THREAT_LEVELS: Record<ThreatLevel, number> = {
  critical: 5,
  high: 4,
  medium: 3,
  low: 2,
...
const DEFAULT_PATTERNS: { pattern: RegExp; category: ThreatCategory; level: ThreatLevel; description: string }[] = [
  // XSS patterns
...
function detectThreats(input: string, patterns: typeof DEFAULT_PATTERNS): Threat[] {
...
export function sanitizeHtml(input: string, dompurify?: { sanitize: (html: string, config?: object) => string }, config?: object): string {
  if (dompurify) {
...
export function sanitize(input: string, type: InputType, dompurify?: { sanitize: (html: string, config?: object) => string }, config?: object): string {
  switch (type) {
    case 'html':
...
export interface FirewallInstance {
  process(input: string, options?: { type?: InputType }): FirewallResult;
...
export function createFirewall(config: FirewallConfig = {}): FirewallInstance {
  ...
  // Helper to log
  const log = (level: ThreatLevel, msg: string) => {
    if (!enabled) return;
  ...
  const instance: FirewallInstance = {
...
export const secureValidators = {
  ...
  email(input: string): { valid: boolean; value: string } {
  ...
  url(input: string): { valid: boolean; value: string } {
  ...
  filename(input: string): { valid: boolean; value: string } {
  ...
  uuid(input: string): { valid: boolean; value: string } {
  ...
  positiveInt(input: string): { valid: boolean; value: number } {
...
export const firewall = {
   ...
   */
  process(input: unknown, options: { type?: InputType; maxLength?: number; area?: string } = {}): FirewallResult {
...
let _config: FirewallConfig | undefined;

// Return type with proper undefined handling
...
function getFirewallConfig(): ResolvedConfig {
  if (!_config) {
    return {
      enabled: true,
      logLevel: 'warn',
...
export function isAreaActive(area: string): boolean {
  return SECURITY_ACTIVE_AREAS[area as keyof typeof SECURITY_ACTIVE_AREAS] ?? true;
}
