 ...
 * USAGE:
 * ```typescript
 * import { sanitizePath, sanitizeInput, createWaf } from '@creadev.org/security/sanitize';
 * 
 ...
 */
export function getClientIP(headers: Headers | Record<string, string>): string | null {
  ...
  for (const headerName of CLIENT_IP_HEADERS) {
    ...
    if (value) {
      if (headerName === 'X-Forwarded-For') return value.split(',')[0].trim();
      return value;
    }
  }
  return null;
